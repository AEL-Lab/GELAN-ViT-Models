...
def register_hooks(model):
    def hook(module, grad_input, grad_output):
        if any(torch.isnan(g).any() or torch.isinf(g).any() for g in grad_input if g is not None):
            print(f"NaN or Inf detected in gradients of {module.__class__.__name__}")
    
    for name, module in model.named_modules():
        if not isinstance(module, nn.Sequential) and not isinstance(module, nn.ModuleList) and len(list(module.children())) == 0:
