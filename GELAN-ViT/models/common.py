...
class ViT(nn.Module):
  	# ViT Block
    def __init__(self, in_chans=16, out_dim=64, model_name='vit_tiny_patch16_224', embed_dim=192, img_size=128, patch_size=4):
        super(ViT, self).__init__()
        self.vit = timm.create_model(model_name, pretrained=False, in_chans=in_chans, img_size=(img_size, img_size), patch_size=patch_size)
        self.vit.head = nn.Identity()  # Remove the classification head

        # Correct the conv_proj layer to match the embedding dimension
        self.conv_proj = nn.Conv2d(self.vit.embed_dim, out_dim, kernel_size=1)  

        # Fixed position embeddings for the largest possible size
        max_num_patches = (img_size // patch_size) ** 2
        self.pos_embed = nn.Parameter(torch.zeros(1, max_num_patches + 1, self.vit.embed_dim), requires_grad=True)

    def forward(self, x):
        B, C, H, W = x.shape
        device = x.device

        # Custom patch embedding to match input size
        patch_size = self.vit.patch_embed.patch_size[0] 
        num_patches = (H // patch_size) * (W // patch_size)
        cls_tokens = self.vit.cls_token.expand(B, -1, -1).to(device)

        x = self.vit.patch_embed.proj(x).flatten(2).transpose(1, 2)

        pos_embed_resized = nn.functional.interpolate(self.pos_embed[:, 1:, :].permute(0, 2, 1), size=(num_patches,)).permute(0, 2, 1).to(device)
        pos_embed_resized = torch.cat([self.pos_embed[:, :1, :].to(device), pos_embed_resized], dim=1)
        ...
        x = x[:, 1:].transpose(1, 2).reshape(B, self.vit.embed_dim, H // patch_size, W // patch_size)
...
class RepNCSPELAN4_ViT(nn.Module):
    # csp-elan
    def __init__(self, c1, c2, c3, c4, c5=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = c3//2
        self.cv1 = Conv(c1, c3, 1, 1)
        self.cv2 = nn.Sequential(RepNCSP(c3//2, c4, c5), Conv(c4, c4, 3, 1))
        self.cv3 = nn.Sequential(RepNCSPViT(c4, c4, c5), Conv(c4, c4, 3, 1))
        self.cv4 = Conv(c3+(2*c4), c2, 1, 1)
